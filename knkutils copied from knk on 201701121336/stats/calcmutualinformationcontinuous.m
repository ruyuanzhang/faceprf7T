...
%   bandwidth along each dimension is equal to half the size of the 
...
%
% history:
% - 2015/11/02 - change bandwidth to half the size of the bins.
%                the main point was to speed up computation.  however,
%                this does change past behavior!  it is not clear
%                what the appropriate sampling strategy is.
%
% example:
...
allpoints = [flatten(m1); flatten(m2)];  % 2 x MANY

% calculate assigment of points to bins
asst1 = ceil((allpoints(1,:) - r1(1)) / sz1);
asst2 = ceil((allpoints(2,:) - r2(1)) / sz2);

% calculate joint entropy
je = zeros(length(mid1),length(mid2));
parfor p=1:length(mid1)
  je0 = zeros(1,length(mid2));
  for q=1:length(mid2)
%    ix = findlocal([mid1(p) mid2(q)],allpoints,[sz1 sz2]);  % very time-consuming
    ix = asst1==p & asst2==q;
    temp = (abs(m1(ix)-mid1(p))/(sz1/2)).^2 + (abs(m2(ix)-mid2(q))/(sz2/2)).^2;  % square of (normalized) distance
    good = find(temp <= 1);
